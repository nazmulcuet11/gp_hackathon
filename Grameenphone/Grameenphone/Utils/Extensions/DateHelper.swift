...
public extension Date {
    ...
    func compare(_ comparison:DateComparisonType) -> Bool {
        ...
        case .isWeekend:
            let range = Calendar(identifier: .gregorian).maximumRange(of: Calendar.Component.weekday)!
    ...
    func adjust(_ component:DateComponentType, offset:Int) -> Date {
        ...
        return Calendar(identifier: .gregorian).date(byAdding: dateComp, to: self)!
    }
    ...
    func dateFor(_ type:DateForType, calendar:Calendar = Calendar(identifier: .gregorian)) -> Date {
        switch type {
        case .startOfDay:
    ...
    func numberOfDaysInMonth() -> Int {
        let range = Calendar(identifier: .gregorian).range(of: Calendar.Component.day, in: Calendar.Component.month, for: self)!
    ...
    internal static func components(_ fromDate: Date) -> DateComponents {
        return Calendar(identifier: .gregorian).dateComponents(Date.componentFlags(), from: fromDate)
    }
...
public enum DateFormatType: CaseIterable {
    public static var allCases: [DateFormatType] {
        return [
            isoYear,
            isoYearMonth,
            isoDate,
            ...
            shortDate,
            shortDateFullYear,
            isoDateTimeWithoutOffset
        ]
    }

    public typealias AllCases = [DateFormatType]
    ...
    var stringFormat:String {
        ...
        case .isoDateTimeWithoutOffset: return "yyyy-MM-dd'T'HH:mm:ss"
